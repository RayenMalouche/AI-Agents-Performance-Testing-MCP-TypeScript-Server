...
class PerformanceTestServer {
  ...
  private async runPerformanceTest(args: any) {
  ...
  private analyzeResponseData(responseData: any, testResult: TestResult) {
  ...
  private async executePerformanceTest(query: string, targetUrl: string, testId: string): Promise<TestResult> {
    const startTime = Date.now();
    const testResult: TestResult = {
      testId,
      timestamp: new Date().toISOString(),
  ...
  private async monitorCosts(args: any) {
    ...
    const limit = timeframeLimits[timeframe as keyof typeof timeframeLimits] || timeframeLimits.day;
  ...
  private async benchmarkScenarios(args: any) {
    ...
    const scenariosToRun = scenarios.includes('all') 
      ...
      : scenarios.filter((s: string) => predefinedScenarios.hasOwnProperty(s));
    
    const benchmarkId = `benchmark_${Date.now()}`;
    const results: any[] = [];
    
    for (const scenario of scenariosToRun) {
      const query = predefinedScenarios[scenario as keyof typeof predefinedScenarios];
  ...
  private async exportResults(args: any) {
  ...
  private getUniqueUrls(results: TestResult[]): string[] {
    const allUrls = results.flatMap(r => r.urlsFetched || []);
    return [...new Set(allUrls)];
  }

  private async generateDetailedReport(analysis: any, results: TestResult[]) {
  ...
  private convertToCSV(results: TestResult[]): string {
    ...
    const rows = results.map(r => 
      `"${r.testId}","${r.timestamp}","${r.query}",${r.responseTime},${r.success},${r.tokensUsed || 0},${r.cost || 0},${r.datasetSize || 0},${r.mcpCallsCount || 0},"${r.errorMessage || ''}"`
    ).join('\n');
    return headers + rows;
  }
